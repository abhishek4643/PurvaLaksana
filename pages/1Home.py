...
try:
    # Use gemini-2.5-flash model with fixed settings (removed temperature/creativity controls)
...
with st.sidebar:
    ...
    # Important Note
    st.warning("AI analysis is for information only. Always consult a doctor.", icon="⚠️")
...
def get_gemini_response(prompt, image=None):
    ...
    try:
        if image:
            # For image analysis, use gemini-2.5-flash
...
def get_chat_response(prompt):
    ...
    try:
        ...
        medical_prompt = f"""
        You are a PurvaLaksana medical AI assistant. Please provide helpful medical information and advice while keeping in mind:
...
def create_pdf_report(report_data, image=None):
    ...
    # Add letterhead
    story.append(Paragraph("PurvaLaksana Medical Center", title_style))
    ...
    Tel: +91-1234567890 | Email: care@purvalaksana.com<br/>
    www.purvalaksana.com
    </para>
    """
    ...
    PurvaLaksana employs state-of-the-art technology, but results should be validated by appropriate medical specialists.</i></para>
    """
...
def find_nearby_hospitals(lat, lon, radius=5000):
    ...
    try:
        ...
        if not response_text:
            st.warning("Hospital search returned empty response. Please try again later.")
            return []
        ...
        st.warning("Could not connect to hospital search service. Please check your internet connection.")
        return []
    except requests.exceptions.Timeout:
        st.warning("Hospital search timed out. Please try again later.")
        return []
    except (ValueError, KeyError) as e:
        st.warning(f"Could not parse hospital data. Please try again later.")
        return []
    except Exception as e:
        st.warning(f"Error finding hospitals: {str(e)}")
        return []
...
def save_analysis_to_database(analysis_data):
    ...
    try:
        ...
        if not response.data:
            st.warning("Failed to save analysis results to database.")
            return False
        ...
        # Handle DNS/network errors gracefully (e.g. [Errno 11001] getaddrinfo failed)
